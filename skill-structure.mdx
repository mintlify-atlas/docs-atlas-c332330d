...
<Note>
  The `metadata:` block in the frontmatter (like `short-description`) is not used by Codex for triggering. Only `name` and `description` determine when a skill activates. Metadata sub-fields exist for other tooling purposes.
</Note>
