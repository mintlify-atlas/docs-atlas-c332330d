...
<Note>
  The `metadata:` sub-block that appears in some skills (e.g., `metadata: short-description: ...`) is **not** used by Codex for triggering or loading decisions. Only `name` and `description` determine when a skill activates. Metadata sub-fields exist for other tooling purposes and can be safely ignored when writing new skills.
</Note>
