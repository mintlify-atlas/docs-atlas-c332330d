---
title: "Vercel Deploy Skill: Ship Apps from Codex"
sidebarTitle: "Vercel"
...
<CardGroup cols={2}>
  <Card title="Framework Detection" icon="magnifying-glass">
    Reads `package.json` to identify the framework (Next.js, Remix, Astro, SvelteKit, Vite, and 40+ others). Detected framework is passed to the deploy API for optimized builds.
  </Card>
  ...
  <Card title="Build Polling" icon="rotate">
    After upload, polls the preview URL every 5 seconds (up to 5 minutes) until it returns a non-5xx status, confirming the build is live.
  </Card>
</CardGroup>
